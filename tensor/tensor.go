...
func (a *Dense) Slice(boundary [][2]int) *Dense {
	for i := range a.dimension {
		if !(boundary[i][0] >= 0 && boundary[i][0] <= a.shape[i]) {
			panic(fmt.Sprintf("%d boundary %d shape %d", i, boundary[i][0], a.shape[i]))
		}
		if !(boundary[i][1] >= boundary[i][0] && boundary[i][1] <= a.shape[i]) {
			panic(fmt.Sprintf("%d boundary %d %d shape %d", i, boundary[i][0], boundary[i][1], a.shape[i]))
		}
	}
...
func Mul(b *Dense, ca complex64, a *Dense) *Dense {
	if !slices.Equal(b.Shape(), a.Shape()) {
		panic(fmt.Sprintf("wrong shape %#v %#v", b.Shape(), a.Shape()))
	}
	// Check if b shares the same backing slice as a.
	if len(Overlap(b.data, a.data)) > 0 {
		for i := range b.dimension {
			if b.viewToAxis[i] != a.viewToAxis[i] {
				panic(fmt.Sprintf("transpose %d", i))
			}
		}
		if b.conj != a.conj {
			panic(fmt.Sprintf("conj %v %v", b.conj, a.conj))
		}
	}
...
func Add(c, a, b *Dense) *Dense {
	...
	if !slices.Equal(c.Shape(), a.Shape()) {
		panic(fmt.Sprintf("wrong shape %#v %#v", c.Shape(), a.Shape()))
	}
	// Check if c shares the same backing slice as a and b.
	for i, other := range []*Dense{a, b} {
		if len(Overlap(c.data, other.data)) == 0 {
			continue
		}
		for j := range c.dimension {
			if c.viewToAxis[j] != other.viewToAxis[j] {
				panic(fmt.Sprintf("transpose %d %d", i, j))
			}
		}
		if c.conj != other.conj {
			panic(fmt.Sprintf("conj %d %v %v", i, c.conj, other.conj))
		}
	}
