...
func (solver *Arnoldi) Solve(a *Dense, opt ArnoldiOptions) (*Dense, *Dense, error) {
	...
	solver.q.Set([]int{0, 0}, solver.randVec(m))
	...
	for _ = range opt.MaxIterations {
		...
		if arnoldiConverged(r, hvecs, eigvals) {
...
func (solver *Arnoldi) iterate(a *Dense, wants, start int) (*Dense, *Dense, *Dense) {
	...
	for ; k <= solver.q.Shape()[1]; k++ {
		...
		if !ok {
			...
			f.Set([]int{0, 0}, solver.randVec(m))
			fNorm, ok = gramSchimdt(f, hk, qk, bufs)
			if !ok {
...
func (solver *Arnoldi) randVec(m int) *Dense {
	b0 := solver.eigvecs.Zeros(m, 1)
	for i := range m {
		b0.SetAt([]int{i, 0}, complex(rand.Float32(), rand.Float32()))
	}
...
func arnoldiConverged(r, vecs, vals *Dense) bool {
	// We can't seem to go below 30*epsilon using float32.
	const tol = 30 * epsilon
